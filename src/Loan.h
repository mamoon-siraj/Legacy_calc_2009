...
public:
  ...
  /**
   * Total loan amount A
   * Implements input validation: Must be non-negative.
   */
  ...
  inline void setInterest(long double i) { 
    if(i < 0) throw std::invalid_argument("Interest rate can't be negative!"); 
